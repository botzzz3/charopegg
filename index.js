...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
      ...
      case '(codorna)':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`( https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS6fwV8-BI85gzUQvtJyVZD9lbmSlbdZWKjNg&usqp=CAU)')
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '(oia que codorna bunita )'})
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
