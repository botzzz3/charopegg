...
const { jts menu} = require('./src/jtsmenu')
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oiin @${num.split('@')[0]}\nBem vindor ao  leia as regras e entre no discord  e veja como joga oara nao cometer ant rp *${mdata.subject}*`
				...
				teks = `+1 foi embora😭 oq ouve @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'trendtwit':
					...
					break
                    case 'jtsmenu':
		client.sendMessage(from, jtsmenu(prefix, sender), text, {quoted: mek})
                break 
                case 'menu':
